	...
	*/
	...
	class Graph {		
		...
		*/
		...
		public function setLegendPosition ($str) {
			$this->legend_position = $str;
		}
	...
	class GoogleGraph {
		...
		*/
		...
		public function printGraph($echoHTML=TRUE, $cache=TRUE) {
			/*
			...
			*/
			...
			if(!empty($this->Graph->legend_position)) 	
				$url .= 'chdlp='.$this->Graph->legend_position.'&';
			...
			if(!empty($this->Graph->chart_size)) {				
				$size = explode('x', $this->Graph->chart_size, 2);
			...
			if ($cache) {
				...
				if ( file_exists($fpath) != TRUE) {
					file_put_contents($fpath,file_get_contents($url));
				}
				
				$url = '/system/cache/charts/'.$fname;
				
			}
			
			// Return
			if ($echoHTML) { 
				echo '<img src="'.$url.'" '.$params.'/>';
				return TRUE;
			} else {
				return $url;
			}
